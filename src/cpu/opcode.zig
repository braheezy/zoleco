...
pub fn lookupBitOpcode(self: *Z80) !void {
    const next_opcode = self.io.readMemory(self.io.ctx, self.pc);
...
pub fn lookupIndexedOpcode(self: *Z80) !void {
    ...
    const prev_opcode = self.io.readMemory(self.io.ctx, self.pc -% 1);
    ...
    if (opcode == 0xCB) {
        ...
        const bitOp = self.io.readMemory(self.io.ctx, self.pc);
        self.pc +%= 1;
...
fn retn(self: *Z80) !void {
    self.pc = Z80.toUint16(self.io.readMemory(self.io.ctx, self.sp + 1), self.io.readMemory(self.io.ctx, self.sp));
    self.sp += 2;
    self.iff1 = self.iff2;
