...
pub fn load_NNMem(self: *Z80) !void {
    const data = try self.fetchData(2);
    const address = Z80.toUint16(data[1], data[0]);
    self.curr_index_reg.?.* = self.io.readMemory(self.io.ctx, address) | (@as(u16, self.io.readMemory(self.io.ctx, address + 1)) << 8);
    self.wz = address +% 1;
    self.q = 0;
}
...
pub fn store(self: *Z80) !void {
    ...
    self.io.writeMemory(self.io.ctx, address, getLowByte(self.curr_index_reg.?.*));
    self.io.writeMemory(self.io.ctx, address + 1, getHighByte(self.curr_index_reg.?.*));
    self.q = 0;
    self.wz = address +% 1;
}
...
pub fn inc_IXD(self: *Z80) !void {
    ...
    const value: u8 = self.io.readMemory(self.io.ctx, address);

    // Step 4: Increment the value with wrapping
    const old_value: u8 = value;
    const new_value: u8 = value +% 1;

    self.io.writeMemory(self.io.ctx, address, new_value);
...
pub fn store_WithDisp(self: *Z80, n: u8) void {
    ...
    self.io.writeMemory(self.io.ctx, address, n);
}
...
fn load_Disp(self: *Z80) u8 {
    ...
    const displacement = self.getDisplacement();
    return self.io.readMemory(self.io.ctx, self.getDisplacedAddress(displacement));
}
...
pub fn load_IXHDsp(self: *Z80) !void {
    ...
    self.register.h = self.io.readMemory(self.io.ctx, address);
    self.q = 0;
}
